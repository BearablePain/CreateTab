//Ждем загрузку всех элементов на странице и переводим в строгий режим 
//Ждать загрузку можно множеством способов(например написать defer)
window.addEventListener('DOMContentLoaded', function () {
  'use strict';

  //Получаем жлементы
  let tab = document.querySelectorAll('.info-header-tab'), //берем все элемеенты менюТабов
  ...
  info.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains('info-header-tab')) { //делегируем событие
      ...
      for (let i = 0; i < tab.length; i++) {
        if (target == tab[i]) { // сравниваем кликнутый менютаб и который нужно показать
